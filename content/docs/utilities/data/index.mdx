---
title: "Data"
description: "Useful utilities to parse and manipulate data"
---
import {
  Bars2Icon,
  Bars3Icon,
  CircleStackIcon,
} from "@heroicons/react/24/solid";

<Cards>
  <Card href="/apis/utilities/data/overview" icon={<CircleStackIcon />}>
    ### Data Overview [!toc]

    Learn about the basics, and how Mesh handles Cardano data
  </Card>

  <Card href="/apis/utilities/data/mesh" icon={<Bars2Icon />}>
    ### Mesh Data [!toc]

    Parse and manipulate data with Mesh Data type
  </Card>

  <Card href="/apis/utilities/data/json" icon={<Bars3Icon />}>
    ### JSON Data [!toc]

    Parse and manipulate data with JSON
  </Card>

  <Card href="/apis/utilities/data/value" icon={<Bars3Icon />}>
    ### Value [!toc]

    Manipulate Value Easily
  </Card>
</Cards>