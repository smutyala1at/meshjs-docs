...
To initialize Hydra with Mesh, you need to set the `HydraProvider` with the Hydra API URL and then use it to initialize the `HydraInstance`. You can use one of the cardano <Link href="/apis/providers">providers</Link>, example: `blockfrostProvider`, or `maestroProvider`, to initialize the `HydraInstance`.
